...
export async function POST(req: NextRequest) {
  try {
    ...
    const fields = {
      ...
      commit4Months: (formData.get("commit4Months") as string) ?? "",
      available5Hours: (formData.get("available5Hours") as string) ?? "",
      timeEffort: (formData.get("timeEffort") as string) ?? "",
      englishComfortable: (formData.get("englishComfortable") as string) ?? "",
